...
const getInvoice = async () => {
    let response = await axios.get(`/api/editInvoice/${route.params.id}`)
    form.value = response.data.invoices
    console.log(form.value.invoice_item)
}
...
const deleteInvoiceItem = (id, i) => {
    ...
    if (id != undefined) {
        axios.get('/api/deleteInvoiceItem/' + id)
    }
}
const addCart = (item) => {
    const itemCart = {
                ...
                <div class="modal__items">
                    <ul style="list-style: none">
                        <li v-for="(item, i) in listProducts" :key="item.id" style="
                            ...
                            <button @click="addCart(item)" style="
